...
save(brt_1, file = "3_output/models/brt_1.rData")
...
bootstrap_brt <- function(data, n_iterations) {
  ...
  for (i in 1:n_iterations) {
    ...
    model <- gbm.step(
      ...
      offset = o_train,
      family = "bernoulli",
      tree.complexity = tuned_param_max$interaction.depth[1],
      n.minobsinnode = tuned_param_max$n.minobsinnode[1],
      learning.rate = tuned_param_max$shrinkage[1],
      bag.fraction = tuned_param_max$bag.fraction[1],
      silent = FALSE,
      plot.main = FALSE
    )
