...
w <- w %>%
  mutate(
    location_buffer_m = ifelse(!is.finite(location_buffer_m),
      0, location_buffer_m
    ),
...
w <- w %>%
  select(
    project_id, survey_type, location,
...
for (i in 1:length(tzs)) {
  ...
  all.i$sunrise <- getSunlightTimes(
    data = all.i, keep = "sunrise",
    tz = tzs[i]
  )$sunrise
...
colnms <- c(
  "organization", "project", "project_id", "survey_type",
...
save(wildtrax_cleaned,
  file = paste0(
    "0_data/manual/response/wildtrax_cleaned_",
    Sys.Date(), ".rData"
  )
)
...
save(wildtrax_cleaned_piwo,
  file = paste0(
    "0_data/manual/response/wildtrax_cleaned_piwo_",
    Sys.Date(), ".rData"
  )
)
